...
const { habitsDb, ObjectId } = require('./db');
...
// GET /api/habits - Return all habits (supports filtering, sorting, projection)
// Query parameters:
...
app.get('/api/habits', async (req, res) => {
  try {
    ...
    // Parse sort from query parameter (default: sort by _id ascending)
    let sort = { _id: 1 };
    if (req.query.sort) {
      try {
    ...
    // Parse projection from query parameter
    let projection = {};
    if (req.query.projection) {
      try {
        projection = JSON.parse(req.query.projection);
      } catch (e) {
        return res.status(400).json({ error: 'Invalid projection format. Must be valid JSON.' });
      }
    }

    const habits = await habitsDb.getAll(filter, sort, projection);
...
app.put('/api/habits/:id', async (req, res) => {
  ...
  try {
    ...
    const existingHabit = await habitsDb.getById(id);
    if (!existingHabit) {
      return res.status(404).json({ error: 'Habit not found' });
    }

    // Update the habit
    const updated = await habitsDb.update(id, title.trim(), description.trim());
    
    if (updated) {
      const updatedHabit = await habitsDb.getById(id);
...
app.delete('/api/habits/:id', async (req, res) => {
  ...
  try {
    ...
    // Delete the habit
    const deleted = await habitsDb.delete(id);
